...
const getTagById = async (tagId) => {
    const response = await db.query(`SELECT name,
                                            sortOrder,
                                            nickname,
                                            tag.creator
                                     FROM tag
...
const updateTag = async (tagId, creatorId, {name, sortOrder}) => {
    const response = await db.query(`UPDATE "tag"
                                     ...
                                     WHERE creator = $3
                                       AND id = $4 RETURNING *`, [name, sortOrder, creatorId, tagId]).catch(err => err)
...
const deleteTag = async (id, creatorId) => {
    const response = await db.query(`DELETE
                                     FROM "tag"
                                     WHERE id = $1
                                       AND creator = $2`, [id, creatorId]).catch(err => err)
...
const findTags = async ({limit, offset, sortByName, sortByOrder}) => {
    const response = await db.query(`SELECT name,
                                     ...
                                     FROM tag
                                              INNER JOIN "user"
                                                         ON "user".uid = tag.creator
                                     WHERE sortOrder = COALESCE($1, sortOrder)
                                       AND name ~ COALESCE($2, name)
                                     LIMIT $3
                                     OFFSET $4`, [sortByOrder, sortByName, limit, offset,]).catch(err => err)

    return new Tag(response).getTags()
}

const countTags = async ({sortByName, sortByOrder}) => {
    const response = await db.query(`SELECT COUNT(*)
                                     FROM tag
                                     WHERE sortOrder = COALESCE($1, sortOrder)
                                       AND name ~ COALESCE($2, name)`, [sortByOrder, sortByName]).catch(err => err)
...
const findMyTags = async (tags, creatorId) => {
    ...
    const response = await db.query(`SELECT *
                                     FROM tag
                                     WHERE creator = $1
                                       AND ${findOr}`, [creatorId, ...tags]).catch(err => err)
